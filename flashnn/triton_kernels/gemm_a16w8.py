...
def triton_gemm_a16w8_forward(out, act, quant_w, scale_w, bias=None, zero_points=None, best_config=None):
    ...
    # sub channel a16w8
        ...
        triton_gemm_a16w8_sub_channel = triton.autotune(
            configs = [best_config] if best_config != None else _get_autotune_configs(is_perchannel=False),
            key=["M", "N", "K"],
        )(_triton_gemm_a16w8_sub_channel_kernel)
        triton_gemm_a16w8_sub_channel[grid](BLOCK_K=k_per_scale, **kwargs)

    return triton_gemm_a16w8_sub_channel.best_config
